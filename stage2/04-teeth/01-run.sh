...
log "TEETH: Install packages required for RPI build"
...
log "TEETH: Building C parts"
# C parts
build_dir=/tmp/teeth/build
mkdir -p /tmp/teeth/build && cd /tmp/teeth/build
...
log "TEETH: Installing service"
# Python parts
service_dir=${ROOTFS_DIR}/opt/teeth
mkdir -p "${service_dir}"
install -m 644 teeth/service/teeth.py "${service_dir}"
install -m 644 teeth/service/requirements.txt "${service_dir}"
